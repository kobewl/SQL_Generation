...
import com.sql.generation.core.builder.sql.MySQLDialect;
import com.sql.generation.core.model.enums.FieldTypeEnum;
...
public class TableSchemaBuilder {
    ...
    /**
     * 根据建表 SQL 构建 TableSchema 对象。
     * 该方法首先会验证 SQL 是否为空，然后使用 MySqlCreateTableParser 解析 SQL，
     * 并根据解析结果填充 TableSchema 对象，包括数据库名称、表名、表注释以及字段信息。
     * 如果解析过程中遇到异常，会抛出 BusinessException。
     *
     * @param sql 建表 SQL 字符串。不可为空。
     * @return 生成的 TableSchema 对象，包含表的结构信息。
     * @throws BusinessException 如果输入的 SQL 为空或解析过程中发生错误。
     */
